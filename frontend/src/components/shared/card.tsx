...
const Card = ({
    ...
    return (
        <MinimalCard
            ...
            {children && <div className="row-start-content col-first_last ml-4">{children}</div>}
            {image && <div className="row-start-content col-side_last mr-4">{image}</div>}
            {priceCircle && <div className={cn("mt-circle-offset-lg ml-circle-offset-lg row-foot_end col-last w-circle-lg h-circle-lg rounded-circle-lg flex items-center justify-center !text-pc bg-gray-300 text-gray-800", colorVariant && colorVariants[colorVariant].dark)}>{priceCircle}</div>}
            {footer && <div className="row-start-foot col-first_last last:pb-2 ml-4">{footer}</div>}
            {actions && <div className="row-start-actions col-first_last flex flex-row gap-4 justify-end self-end border-t py-2 ml-4">{actions}</div>}
        </MinimalCard>
    )
}
